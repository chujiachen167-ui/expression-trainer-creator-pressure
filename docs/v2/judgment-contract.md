...
日期：2026-09-21。判断器版本：`rules-v1.2.0`。种类：`rules`（规则判断器，不是语义模型，也不是平台推荐接口）。
...
相对模拟兴趣：按所选平台（抖音、快手、小红书、YouTube、Instagram、B站、视频号）的**公开创作者教育**对齐文案层启发式。不是百分号留存概率，不是真实观看率，也不是任何平台后台完播率或推荐分。拿不到后台数据就不编。
