...
| 层级 | 训练目标 | 已实现 |
|---|---|---|
| V1 镜头基线 | 面对镜头自然、紧凑地表达 | 内置/外接摄像头与麦克风选择、可选本地录制与回看、透明实时字幕、中英混合/English 两种模式、原词库指标、训练规则、AI 配置与报告 |
...
首页页脚的“联系与反馈”会进入 `contact.html`。该页保留三张联系方式原图，并以原生 JavaScript 适配 React Bits 的 Depth Carousel 视觉机制：中心卡片完整展示、两侧提供深度预览，支持按钮、键盘、拖动和低干扰自动轮换。
...
“设备与录制”会列出系统已连接的摄像头与麦克风。设备名称通常在用户主动授予权限后才显示；训练开始后会锁定设备，避免录制途中切换导致文件损坏。桌面端 Sherpa 转写与录像共用所选麦克风口径；浏览器 Web Speech 的实际输入设备仍由浏览器和操作系统决定。

录像默认关闭。只有用户主动开启“同时录制本轮”后才会请求录制画面和声音；停止训练后生成当前页面内的临时回看，可连同本轮逐字稿检查并手动保存为 WebM。录像不进入项目配置、不自动上传，也不会在刷新或关闭页面后继续保留。桌面版通过系统保存窗口写入“视频”目录，浏览器版使用普通下载。
