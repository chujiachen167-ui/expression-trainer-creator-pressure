...
七个分页共用调控板底部的固定保存栏，不需要逐页保存：
- **保存全部参数到项目**：包含功能开关、主题、布局、所有组件、文案、fineTune 元素样式与 extraCopy 内容修改，以及同一份草稿中已有的其他页面配置。桌面端直接写入仓库根目录 `creator-project-config.js`；浏览器端让用户选择文件，或下载该文件。浏览器选中其他目录不代表已写回仓库，必须替换项目根目录同名文件后再提交 Git。
- **导出全部参数 JSON**：把上述同一份完整配置导出为便于备份和交接的 JSON 文件。导出不等于写入项目，也不等于提交 GitHub。
