 ...
 * 简单中文分词（基于最大正向匹配 + 词表）
 */
function parseCustomFillers(customWords) {
  return String(customWords || '').split(/[，,、\s]+/).map(word => word.trim()).filter(Boolean);
}

function segmentText(text, customFillers = []) {
  const words = [];
  let i = 0;
  const maxLen = 6;
...
function analyzeText(text, options = {}) {
  ...
  const customFillers = parseCustomFillers(options.customWords);
  const fillerSet = new Set([...FILLER_WORDS, ...customFillers]);
  const words = segmentText(text, customFillers);
...
module.exports = { loadLexicon, analyzeText, VAGUE_TO_PRECISE, FILLER_WORDS, HEDGE_WORDS, parseCustomFillers };
