...
function getASRStatus() {
  ...
  const missingFiles = requiredFiles.filter(file => !fs.existsSync(path.join(modelDir, file)));
  return {
    engine: 'sherpa-onnx',
    model: MODEL_SUBDIR,
...
async function initASR() {
  ...
  const config = {
    ...
    maxActivePaths: 4,
    enableEndpoint: true,
    // Creator口播需要更及时的字幕落句；保持保守阈值，避免过度切碎长句。
...
module.exports = { initASR, feedAudio, stopRecognition, getASRStatus };
