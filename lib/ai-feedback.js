...
const { getRealtimePrompt, getReportPrompt, getOptimizedScriptPrompt } = require('./prompts');
...
async function callAPI(endpoint, apiKey, model, messages, maxTokens = 200) {
  ...
  const content = data?.choices?.[0]?.message?.content
    ...
    || data?.response
    || '';
  const script = String(content || '').replace(/<think>[\s\S]*?<\/think>/gi, '').trim();
  if (!script) throw new Error('优化服务没有返回可用内容，请重试。');
  return script;
}
...
async function sendOptimizedScript(fullText, settings, customPrompt) {
  const config = getProviderConfig(settings);
  const prompt = getOptimizedScriptPrompt(fullText, customPrompt);
  return callAPI(config.endpoint, config.apiKey, config.model, [
    { role: 'system', content: prompt.system },
    { role: 'user', content: prompt.user }
  ], 2400);
}
...
module.exports = { sendFeedback, sendReport, sendOptimizedScript, testConnection };
