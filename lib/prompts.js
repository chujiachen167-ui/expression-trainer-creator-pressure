...
function getOptimizedScriptPrompt(fullText, customPrompt) {
  let styleBlock = '';
  if (customPrompt?.styleRef) styleBlock += `\n用户希望保留的表达风格：${customPrompt.styleRef}`;
  if (customPrompt?.goals) styleBlock += `\n本轮表达目标：${customPrompt.goals}`;
  return {
    system: `你是专业的中文口播稿编辑。请把用户的口语逐字稿整理成一份可以直接开口说的优化台词稿。
...
6. 只输出最终优化台词稿，不解释修改过程，不输出评分。${styleBlock}`,
    user: `请优化下面这份逐字稿：\n\n---\n${String(fullText || '').slice(0, 6000)}\n---`
  };
}

module.exports = { getRealtimePrompt, getReportPrompt, getOptimizedScriptPrompt };
