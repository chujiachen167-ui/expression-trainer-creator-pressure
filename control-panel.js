(() => {
  ...
  function collectCopyTargets() {
    const candidates = [...document.querySelectorAll(copySelector)].filter(node => {
      if (node.closest('.qa-panel, .qa-trigger, .reading-transition')) return false;
  ...
  function applyCopy() {
    ...
    document.title = state.copy[titleKey] ?? document.documentElement.dataset.qaTitleDefault ?? document.title;
  ...
  function apply() { applyTheme(); applyFeatures(); applyElements(); applyCopy(); }
  ...
  function addPanel() {
    ...
    panel.innerHTML = `<div class="qa-panel-head"><div><strong>人工验收调控板</strong><small>配置保存在此浏览器</small></div><button type="button" class="qa-close" aria-label="关闭调控板">×</button></div>
      <div class="qa-scroll">
        ...
        <section class="qa-copy-section"><h2>本页文案库</h2><p class="qa-hint">所有静态可见文案都能在这里改。修改会立即预览并保存；多行内容可直接换行。</p><label class="qa-copy-field"><span>浏览器标签标题</span><input data-copy-document-title></label><div class="qa-copy-list" data-copy-list></div></section>
    ...
    const renderCopyFields = () => {
      ...
      list.innerHTML = items.map(({ key, label, defaultText }) => `<label class="qa-copy-field"><span>${label}</span><textarea data-copy-key="${key}" rows="2"></textarea></label>`).join('');
      ...
      const titleField = panel.querySelector('[data-copy-document-title]');
      titleField.value = state.copy[titleKey] ?? document.documentElement.dataset.qaTitleDefault ?? document.title;
    };
    ...
    trigger.addEventListener('click', () => { panel.hidden = !panel.hidden; if (!panel.hidden) renderCopyFields(); trigger.setAttribute('aria-expanded', String(!panel.hidden)); });
    ...
    renderElementFields(); renderCopyFields(); refreshJson(); refreshAvatarProvider();
  }
